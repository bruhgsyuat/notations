...
export class BlobsNotation extends Notation {
  ...
  formatNegativeUnder1000(num) {
    return `${NEGATIVE}${this.blobify(new Decimal(num - 1))}`
  }

  formatNegativeDecimal(num) {
  ...
  blobify(num) {
    ...
    if (Math.floor(number / LEN) < LEN + 1) {
      return (
        BLOBS[Math.floor(number / LEN) - 1] + BLOBS[Math.floor(number % LEN)]
      )
    }
    return (
      this.blobify(Decimal.floor(number / LEN - 1)) +
      BLOBS[Math.floor(number % LEN)]
    )
  }
